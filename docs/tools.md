...
| `list_dir` | Workspace | Read | Inspect files in the thread work directory | Discover files before reading/editing/running commands |
| `read_file` | Workspace | Read | Read a text file from the thread work directory | Inspect generated or downloaded files |
...
| `run_shell` | Workspace/runtime | External | Run a focused shell command in the thread work dir | Rendering, conversion, inspection, simple local automation |
...
| `download_attachment` | Outline attachment | External | Download an Outline attachment or URL into the thread work dir | Read or transform user-provided files |
...
| Thread-local file workspace | `list_dir`, `read_file`, `write_file`, `edit_file` | Restricted to the thread `work/` dir |
...
- The local workspace is thread-scoped, so generated files are isolated per comment thread.
