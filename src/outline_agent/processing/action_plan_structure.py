...
def preview_action_plan_step(tool_name: str, args: dict[str, Any]) -> str:
    if tool_name == "ask_web_search":
        return f"ask_web_search[{truncate(str(args.get('query') or '?'), 60)}]"
...
def describe_action_step_for_progress(tool_name: str, args: dict[str, Any]) -> str:
    ...
    if tool_name == "ask_web_search":
        return f"look up web information for `{preview(str(args.get('query') or '?'), 80)}`"
